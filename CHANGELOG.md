...
### 🛠 Feature Release — 15 Tools

### Added
- **6 New Security Tools (total: 15):**
  - 🌐 **DNS Lookup** (adizendns.py) — A, AAAA, MX, NS, TXT, CNAME, SOA, PTR with full dnspython support and socket fallback
  - 🔒 **SSL/TLS Inspector** (adizenssl.py) — Certificate validity, expiry countdown, cipher suite, protocol, and A–F security rating
  - 🔑 **Password Strength Analyzer** (adizenpassword.py) — Entropy-based scoring, crack time estimate, NIST-aligned recommendations, common password detection
  - 🔎 **Subdomain Enumerator** (adizensubdomain.py) — Multi-threaded DNS brute-force with 80-entry built-in wordlist + custom wordlist support
  - 📋 **CVE Search** (adizencve.py) — Exact CVE-ID lookup (CIRCL API) and keyword search (NVD), with CVSS scores and severity labels
  - 💻 **Reverse Shell Detector** (adizenrevshell.py) — 54-pattern static analysis covering Bash, Python, PHP, PowerShell, Perl, Ruby payloads
- **Desktop GUI updated** to show all 15 tools in sidebar
- **requirements.txt** updated to include optional `dnspython` dependency

### Changed
- Window title updated to reflect 15-tool count
